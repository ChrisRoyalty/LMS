...
export default function Courses() {
  ...
  const anyModalOpen = modalOpen || !!confirmId
  useEffect(() => {
    if (anyModalOpen) {
  ...
  function openEdit(course: Course) {
    ...
    setCategory(cat === 'intermediate' ? 'intermediate' : cat === 'advanced' ? 'advanced' : 'beginner')
                      ...
                      <tr key={c.id} className="border-b border-neutral-200 last:border-0">
                        ...
                        <Td className="text-center">
                          <div className="flex items-center justify-center gap-3">
                            <button className="rounded-md p-2 hover:bg-neutral-100" aria-label="Edit" onClick={() => openEdit(c)}>
                              <SquarePen className="h-4 w-4 text-neutral-700" />
                            </button>
                            <button className="rounded-md p-2 hover:bg-neutral-100" aria-label="Delete" onClick={() => setConfirmId(c.id)}>
          ...
          <Modal onClose={() => !working && setModalOpen(false)} title={modalMode === 'create' ? 'Add New Course' : 'Edit Course'}>
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button type="button" className="btn-secondary" onClick={() => setModalOpen(false)} disabled={working}>Cancel</button>
        ...
        {confirmId && (
          <Modal onClose={() => !working && setConfirmId(null)} title="Delete Course">
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              ...
              <button className="btn-danger" onClick={() => onDelete(confirmId)} disabled={working}>Delete</button>
            </div>
          </Modal>
        )}
      </div>
...
// Replace your Modal with this portal-based version
...
function Modal({
  ...
  // Create a dedicated modal root in <body> (once)
  const el = useMemo(() => {
  ...
  const node = (
    <>
      ...
      <div
        className="fixed inset-0 z-[10000] h-screen w-screen bg-black/40 backdrop-blur-[2px]"
        onClick={onClose}
        aria-hidden="true"
      />
      {/* Centered dialog */}
