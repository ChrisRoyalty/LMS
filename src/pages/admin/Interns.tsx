...
export default function Interns() {
  ...
  const internRoleId = useMemo(
    () => roles.find(r => r.name?.toLowerCase() === 'intern')?.id || '',
    [roles]
  )
  ...
  async function preload() {
    setLoading(true)
    try {
      const [usersRes, coursesRes, batchesRes, rolesRes] = await Promise.all([
      ...
      const interns = users.filter(u => (u.role?.name || '').toLowerCase() === 'intern')

      const mapped: Row[] = interns.map(u => {
        const titles =
      ...
      showToast({ kind: 'error', title: 'Could not load interns', message: e?.response?.data?.message || 'Please try again.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  async function onCreate(e: React.FormEvent) {
    e.preventDefault()
    if (!internRoleId) {
      showToast({ kind: 'error', title: 'Missing role', message: 'Intern role not found. Please create/enable the role.' })
      return
    }
    setCreating(true)
    try {
      const body = {
        ...
        batchId: cBatchId || undefined,
      }
      const { data } = await api.post('/api/user/register-user', body)
      showToast({ kind: 'success', title: 'Intern created', message: data?.message || 'User created' })
      setCreateOpen(false)
      await preload()
    } catch (err: any) {
  ...
  function openEdit(r: Row) {
    setEditRow(r)
    setEName(r.name); setEEmail(r.email)
    setECourseId(r.courseId || '')
    setEBatchId(r.batchId || '')
    setERoleId(r.roleId || internRoleId || '')
    setEditOpen(true)
  }
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      const body = {
        fullName: eName,
        email: eEmail,
        roleId: eRoleId || internRoleId || undefined,
        courseId: eCourseId || undefined,
        batchId: eBatchId || undefined,
      }
      const { data } = await safeEditUser(editRow.id, body)
      showToast({ kind: 'success', title: 'Intern updated', message: data?.message || 'Changes saved' })
      setEditOpen(false)
      await preload()
    } catch (err: any) {
  ...
  async function onDelete() {
    ...
    try {
      ...
      showToast({ kind: 'success', title: 'Intern deleted', message: data?.message || 'Removed successfully' })
      setConfirmId(null)
      await preload()
    } catch (err: any) {
    ...
    <div className="space-y-4 max-w-[1200px] mx-auto">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-neutral-500">Manage all interns in the academy</p>
        </div>
        ...
        <CardContent className="overflow-hidden">
          ...
          <div className="grid gap-3 md:hidden">
            {loading ? (
              ...
              <div className="py-8 text-center text-neutral-500">No interns found.</div>
            ) : (
              filtered.map(r => (
                <div key={r.id} className="rounded-2xl border border-neutral-200 bg-white p-3">
                  ...
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    <div className="flex flex-wrap gap-2">{renderCourseChips(r.courseTitles)}</div>
                    {r.batchName && <span className="text-xs text-neutral-700">• {r.batchName}</span>}
                  </div>
            ...
            <table className="w-full min-w-[980px] table-fixed border-separate border-spacing-0">
              <thead className="sticky top-0 bg-white">
                <tr className="text-left text-neutral-700">
                  <Th className="w-[220px]">Intern</Th>
                  <Th className="w-[260px]">Email</Th>
                  <Th className="w-[340px]">Assigned Courses</Th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={7} className="py-10 text-center text-neutral-500">No interns found.</td></tr>
                ) : (
                  filtered.map(r => (
      ...
      {/* Create modal */}
      {createOpen && (
        <Modal title="Add New Intern" subtext="Add a new intern and assign course & batch" onClose={() => !creating && setCreateOpen(false)}>
      ...
      {/* Edit modal */}
      {editOpen && editRow && (
        <Modal title="Edit Intern" onClose={() => !saving && setEditOpen(false)}>
      ...
      {/* Delete confirm */}
      {confirmId && (
        <Modal title="Delete Intern" onClose={() => !deleting && setConfirmId(null)}>
          <p className="text-sm text-neutral-700">Are you sure you want to delete this intern? This action cannot be undone.</p>
...
.btn-primary { @apply inline-flex items-center gap-2 rounded-xl bg-[#0B5CD7] px-4 py-2 text-white hover:brightness-95; }
.btn-secondary { @apply inline-flex items-center gap-2 rounded-xl border border-neutral-200 bg-white px-4 py-2 text-neutral-700 hover:bg-neutral-50; }
*/
